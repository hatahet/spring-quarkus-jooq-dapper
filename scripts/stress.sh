...
usage() {
  echo "Usage: $0 [--dry-run] <application.jar|application.dll>"
  echo "The application path is relative to your current directory, or absolute."
  echo "JARs use Java; DLLs use .NET. --dry-run prints the launch command only."
}
...
if [[ $# -ne 1 || ! -f "$1" ]]; then
  echo "Error: supply the path to an existing application JAR or DLL." >&2
  usage >&2
  exit 1
fi
...
case "$artifact" in
  *.jar)
    launch=(java -XX:+UseParallelGC -XX:ActiveProcessorCount=4 -Xms512m -Xmx512m -jar "$artifact")
    ;;
  *.dll)
    launch=(env DOTNET_PROCESSOR_COUNT=4 dotnet "$artifact")
    ;;
  *)
    echo "Error: expected a .jar or .dll application." >&2
    exit 1
    ;;
esac
...
runtime=java
[[ "$artifact" == *.dll ]] && runtime=dotnet
if ! command -v "$runtime" >/dev/null 2>&1; then
  echo "Error: $runtime is not installed or is not on PATH." >&2
  exit 1
fi
...
# DOTNET_PROCESSOR_COUNT provides the equivalent processor-count hint for .NET.
# These hints do not pin either application to CPU cores.
"${launch[@]}" &
CURRENT_PID=$!
