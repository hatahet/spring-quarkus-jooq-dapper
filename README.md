...
This application is designed to allow like-for-like performance comparisons between Spring Boot, Quarkus, and ASP.NET Core.
...
- **Parity**
    - Every implementation exposes the same API, uses the same domain shape, runs the same SQL workload, and reads the same PostgreSQL schema and seed data. The Java implementations use jOOQ; ASP.NET Core uses Dapper.
...
- [aspnet10](aspnet10)
    - An ASP.NET Core implementation targeting .NET 10 and using Dapper

The Spring compatibility module remains a legacy ORM-based variant and is not part of the jOOQ/Dapper comparison set.
...
The Java modules can be built using:

```sh
./mvnw clean verify
```

You can also run `./mvnw clean verify` at the project root to build all Java modules.

Build and test the ASP.NET Core module with:

```sh
dotnet test aspnet10/AspNet10.slnx
```
